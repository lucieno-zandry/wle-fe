...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  const { lang } = params;
  if (response && response.data?.is_taken) return email;
...
export default function () {
  ...
  const handleValidationErrorsChange = (validationErrors: string[] | null, e: React.FocusEvent<HTMLInputElement>) => {
        ...
        if (error instanceof ValidationException) {
          ...
          for (const [field, messages] of Object.entries(error.errors)) {
            translated[field] = messages.map(msg => {
              ...
              if (msg.includes('email')) return t('validation.email_invalid');
              return msg;
            });
          }
          return setFormValidationErrors(translated);
        }
        toast.error(t('failed_to_login', { status: error.status }), { description: error.data?.message });
      })
